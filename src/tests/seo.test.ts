...
describe("SEO — social cards", () => {
  ...
  it("points og:image and twitter:image at a real 1200×630 PNG", () => {
    for (const key of ["og:image", "twitter:image"]) {
      expect(metaContent(key)).toBe(`${DOMAIN}/og-image-v3.png`);
    }
    ...
    expect(existsSync(join(ROOT, "public/og-image-v3.png"))).toBe(true);
    expect(pngSize("public/og-image-v3.png")).toEqual({ width: 1200, height: 630 });
  });
});
