    ...
    <header className="flex items-center justify-between gap-3 py-4">
      <div className="flex items-center gap-3">
        ...
        <div className="leading-tight">
          <p className="text-base font-bold text-card-foreground">{t("brandEn")}</p>
          <p className="text-sm text-muted-foreground">{t("brandBn")}</p>
        </div>
      </div>

      <nav aria-label={t("langLabel")} className="flex rounded-full border border-border bg-card p-0.5">
        {options.map((opt) => (
          <button
            key={opt.id}
            type="button"
