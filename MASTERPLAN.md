...
- [x] **Phase 11 — Programmatic conversion pages (2026-09-26)**: 36 static long-tail pages (curated pairs + reverses) generated at build time by a Vite plugin from `src/lib/seoPages.ts` — self-canonical HTML with unique metadata, exact rate, registry-verified conversion table, bilingual FAQPage schema, calculator deep links, reciprocal reverse links; generated sitemap (homepage + 36 pages) replaces the hand-maintained one; homepage no-JS content links five highest-intent pages. Dev middleware serves identical HTML. 149 tests (incl. 18 new page contracts).
...
| 24 | Programmatic conversion pages, curated not exhaustive | 36 static pages at build time from a hand-curated pair list (`src/lib/seoPages.ts`) plus automatic reverses — not all 110 ordered pairs. Rationale: pages must map to a real question people ask ("১ কাঠা সমান কত ডেসিমেল"); exhaustive generation would produce thin near-duplicates that attract no queries and dilute the site. Pages are plain HTML (no framework JS), self-canonical, carry bilingual rate + table + FAQ schema, deep-link into the calculator with pre-filled state, and cross-link their reverse. Generated at build via a Vite plugin writing `dist/<slug>/index.html`; dev middleware serves the same HTML so local/preview behaviour matches production. Sitemap is generated from the same list, so pages and sitemap can never drift apart. |
